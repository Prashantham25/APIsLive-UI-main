...
function Prospectsimport() {
  ...
  const UploadDocument = async (file) => {
    ...
    await UploadFiles(formData).then((result) => {
      if (result.data[0].fileName !== null) {
        newCrm.Documents[0].fileName = result.data[0].fileName;
        // setnewCrm((prevState) => ({ ...prevState, fileName: result.data[0].fileName }));
        setNewCrm((prevState) => ({ ...prevState, newCrm }));
        console.log("Documentfile", result.data[0].fileName);
        console.log("jsonbinding", newCrm);
  ...
  const handleDeleteFile = async (fileName) => {
    await DeleteFile(fileName).then((result) => {
      ...
      if (result.data.status === 5) {
        newCrm.Documents[0].fileName = "";
        // if (type === "fileName") {
        //   setdocumentType();
        setNewCrm((prevState) => ({ ...prevState, newCrm }));
        console.log("jsonbindingdeleted", newCrm);
        // }
      }
    });
  };
  ...
  return (
    <MDBox p={2} height="500">
      <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
        <MDBox display="flex" flexDirection="row" alignItems="center">
          <IconButton size="small">
            <ArrowBackIcon sx={{ cursor: "pointer" }} onClick={handleBackProspects} />
          </IconButton>
          <MDTypography
      ...
      <Stack spacing={2}>
        <Card maxHeight="100%" sx={{ height: "673px" }}>
          <MDTypography
            style={{
              fontSize: "24px",
              variant: "h3",
          ...
          >
            ...
            >
              {/* {newCrm.ProspectDetailsJson.Documents[0].fileName !== ""
                ? newCrm.ProspectDetailsJson.Documents[0].fileName
                : null}{" "} */}
              {newCrm.Documents[0].fileName !== "" ? newCrm.Documents[0].fileName : null}{" "}
              {newCrm.Documents[0].fileName !== "" ? (
                <MDTypography
                  style={{
                  ...
                  onClick={() => handleDeleteFile(newCrm.Documents[0].fileName)}
                >
                  {" "}
                  remove{" "}
                </MDTypography>
            ...
            {upload === true && (
              <MDButton
                sx={{ borderRadius: "1px", ml: "17rem", mt: "1" }}
                onClick={handleModalUploadImportOpen}
              >
                Upload
              </MDButton>
            )}
            <MDTypography sx={{ fontSize: "0.7rem", ml: "8rem", mt: "-10px", color: "#FF0000" }}>
          ...
          > */}
          <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
            <MDTypography variant="h5" sx={{ fontSize: "16px", ml: "40rem", mt: "-15rem" }}>
              Don&apos;t have a file ready?
            </MDTypography>
            {/* <MDTypography sx={{ fontSize: "0.7rem", ml: "40rem", mt: "1rem" }}> */}
            <MDTypography sx={{ fontSize: "14px", ml: "35rem", mt: "1rem" }}>
              Download sample template and add your data to it,
              <br />
              {` or apply the format to your own
              spreadsheet.`}
            </MDTypography>
            {/* <MDTypography sx={{ fontSize: "14px", ml: "40rem" }}></MDTypography> */}
            <MDButton
              variant="outlined"
              sx={{ ml: "40rem", mt: "1rem" }}
