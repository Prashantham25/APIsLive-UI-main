...
import Summary from "assets/images/BrokerPortal/Summary.png";
                ...
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <DesktopDatePicker
                    ...
                    renderInput={(params) => (
                      <MDInput
                        {...params}
                        sx={{ width: "100%", "& .MuiFormLabel-asterisk": { color: "red" } }}
                        variant="standard"
                        required
...
function InputSummary() {
  ...
  const handleProceed = async () => {
    // if (
    //   selected.InsuranceCompanyName === null ||
    //   selected.PolicyNumber === null ||
    //   selected.ODPolicyEndDate === null
    // ) {
    //   swal({
    //     icon: "error",
    //     text: "Please fill all the fields.",
    ...
    if (POSPSales === "POSP") {
      ...
      await FetchPOSPDetails(Email).then(async (result) => {
        ...
        quoteInput.POSPInfo.partnerId = result.data[0].partnerID;
  ...
  useEffect(() => {
    const newValue = isNewBusiness ? 3 : 1;
    ...
    <MDBox>
      ...
      <Grid container spacing={3} sx={{ px: "2rem", pt: "0.5rem", mt: -0.5 }}>
        ...
        <Grid item xs={12} sm={12} md={5} lg={5} xl={5} xxl={5}>
          <MDBox component="img" src={Summary} sx={{ width: "100%" }} />
        </Grid>
