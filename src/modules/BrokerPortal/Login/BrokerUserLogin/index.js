...
// import HDFCLogo from "assets/images/logos/iNube-small.png";
import AMLogo from "assets/images/BrokerPortal/CompanyLogos/A&MLogo.png";
...
import LoginMobile from "./BULoginMobile";
// import {
//   images,
//   setLogo,
//   setCustTheme,
...
function BrokerUserLogin() {
  ...
  const handleClose = () => {
    ...
    setOpen(false);
    navigate(`/modules/BrokerPortal/Pages/BrokerUserLanding`);
    // fetchUser();
  };
    ...
    if (flags.EmailValidationFlag === false && flags.otpValidationFlag === false) {
      ...
      await GetOTP(dispatch, verifyOTP).then((results) => {
        ...
        if (results.status === 1) {
          ...
          localStorage.setItem("BrokerUser", "Broker");
          setOpen(true);
  ...
  const handleLoginWithUserName = async () => {
    ...
    if (loginDetails.Email === "") {
      ...
      swal({
        icon: "error",
        text: "Please enter Registered username",
      });
    } else {
      setFlags((prevState) => ({ ...prevState, EmailValidationFlag: false }));
    }
    ...
    if (flags.passwordValidationFlag === false && flags.EmailValidationFlag === false) {
      ...
      await GetLOGIN(dispatch, loginDTO).then((results) => {
        ...
        if (results.status === 4 && results.firstName !== "") {
          swal({
            icon: "error",
  ...
  const onForgotPassword = () => {
    if (loginDetails.Email === "") {
      ...
      //   swal({
      //     icon: "error",
      //     text: "Please enter register EmailID",
      //   });
      // }
    }
  };
                  ...
                  <Grid item xs={12} sm={12} md={8} lg={8} xl={8}>
                    ...
                    >
                      <Grid
                        ...
                        alignContent="center"
                      >
                        {/* <MDBox component="img" src={bgLoginImage} sx={{ width: "35rem" }} /> */}
                        <MDBox
                          minHeight="5vh"
                          width="18rem"
                          sx={{
  ...
  return mobileView === true ? <LoginMobile /> : getReturnValue();
}

export default BrokerUserLogin;
