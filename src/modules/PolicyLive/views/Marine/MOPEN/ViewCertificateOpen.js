...
function ViewCertificateOpen() {
  const [res, setRes] = useState([]);
  const { search } = useLocation();
  const query = new URLSearchParams(search);
  const CertificateNo = query.get("CertificateNo");
  console.log("policyno", CertificateNo);
  useEffect(async () => {
  ...
  const ViewCertificate = [
    ...
    {
      type: "OtherLoading",
      label: "Enter the Port of Loading",
      option: autocomplete,
      value: res.PortOfLoading1 || "",
    },
    {
      type: "Input",
    ...
    },
    {
      ...
      label: "Enter the Port of Destination*",
      option: autocomplete,
      value: res.PortOfDestination1 || "",
    },
    ...
    { type: "Input", label: "Consgin Name", value: res.ConsigneeName || "" },
    { type: "Input", label: "Consgin Address", value: res.ConsigneeAddress || "" },
    { type: "Input", label: "Consgin Contact No", value: res.CosigneeContactNo || "" },
    ...
    { type: "Input", label: "Settling Agent", value: res.SettlingAgent || "" },
    {
      ...
      value: res.BalanceSIBeforeCurrentCertificate || "",
    },
    {
      type: "Input",
      label: "Balance Permium Before Current Certificate",
      value: res.BalancePremiumBeforeCurrentCertificate || "",
    },
          ...
          <Grid container spacing={2} ml={2} mb={2}>
            {ViewCertificate.map((item) => {
              switch (item.type) {
                ...
                case "OtherLoading":
                  return res.PortOfLoading === "Others" ? (
                    <Grid item xs={12} sm={12} md={2.8} lg={2.8} xl={2.8} xxl={2.8}>
                      <MDInput label={item.label} name={item.name} value={item.value} disabled />
                    </Grid>
                  ) : null;
                case "OtherDestination":
                  return res.PortOfDestination === "Others" ? (
                    <Grid item xs={12} sm={12} md={2.8} lg={2.8} xl={2.8} xxl={2.8}>
                      <MDInput label={item.label} name={item.name} value={item.value} disabled />
                    </Grid>
                  ) : null;
...
export default ViewCertificateOpen;
