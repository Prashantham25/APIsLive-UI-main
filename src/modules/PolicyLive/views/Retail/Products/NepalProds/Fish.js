...
import FishExcelDownload from "assets/images/Nepal/Fish_LivestockDetails_ExcelUpload.xlsx";
  ...
  switch (activeStep) {
    case 0:
      steps = [
        ...
        { name: "Fish Information", visible: true },
...
const getSectionContent = ({ activeStep, dto, setDto, masters, setMasters }) => {
  ...
  const handleDataGrid = (e) => {
    ...
    if (e.target.name === "NumberOfFish") {
      // const Regex = /^[0-9]*$/;
      if (IsNumeric1(e.target.value) === true) {
        flag.NumberofFish = false;
        setFlag({ ...flag });
        objectPath.set(lDto, "Temp.DataGridValues.0.NumberOfFish", e.target.value);
        objectPath.set(lDto, "InsurableItem.0.RiskItems.0.NumberOfFish", e.target.value);
        setDto({ ...lDto });
      } else {
        flag.NumberofFish = true;
        setFlag({ ...flag });
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (
      ...
      lDto.Temp.DataGridValues[0].NumberOfFish !== undefined
    ) {
      lDto.InsurableItem[0].RiskItems = [{ ...RiskItemFish() }];
      lDto.InsurableItem[0].RiskItems[0].NumberOfFish = lDto.Temp.DataGridValues[0].NumberOfFish;
      lDto.InsurableItem[0].RiskItems[0].SumInsured = lDto.Temp.DataGridValues[0].SumInsured;
    }
    if (activeStep === 2 && lDto.Temp.Upload.length > 0) {
    ...
    if (activeStep === 2 && lDto.InsurableItem[0].RiskItems.length > lDto.Temp.Upload.length) {
      lDto.Temp.Upload = lDto.InsurableItem[0].RiskItems;
      setDto({ ...lDto });
    }
  }, [activeStep]);
  ...
  const downloadFile = () => {
    ...
    link.download = "Fish_LivestockDetails_ExcelUpload";
    link.href = FishExcelDownload;
    link.click();
  };
  ...
  const ColumnInfo = {
    InsuredName: {
      mandatory: true,
      mandatoryErrMes: "(Insured Name is mandatory field)",
      validationErrMes: () => "",
      ValidationFun: IsFreetextNoSpace,
    },
    InsuredAddress: {
      mandatory: true,
      mandatoryErrMes: "(Insured Address is mandatory field)",
      validationErrMes: () => "",
      ValidationFun: IsFreetextNoSpace,
    },
    NumberofFish: {
      mandatory: true,
      mandatoryErrMes: "(Number of fish is mandatory field)",
      validationErrMes: () => "(Number of fish field accepts  numeric values only)",
      ValidationFun: IsNumeric,
    },
    AgeofFish: {
      mandatory: true,
      mandatoryErrMes: "(Age of fish is mandatory field)",
      validationErrMes: () => "",
      ValidationFun: IsFreetextNoSpace,
    },
    BreedCasteVariety: {
    ...
    TypeOfFish: {
      mandatory: true,
      mandatoryErrMes: "(Type of fish is mandatory field)",
      validationErrMes: () => "()",
      ValidationFun: IsFreetextNoSpace,
    }, // mandatory
    AreaOfPond: {
  ...
  const onClear = () => {
    if (rowSelectionModel.length !== 0) {
      ...
      // const FormatedDatafilteredList = lDto.FormatedData.CalculatedPremiumDetails.output.filter(
      //   (item) => !rowSelectionModel.includes(item.row_id)
      // );
      ...
      // lDto.FormatedData.CalculatedPremiumDetails.output = FormatedDatafilteredList;
      ...
      objectPath.set(lDto, "NumberofFish", lDto.Temp.Upload.length);
      objectPath.set(
        lDto,
        "FormatedData.CalculatedPremiumDetails.NumberofFish",
  ...
  const handlePremium = async (successRecord) => {
    ...
    if (successRecord.length !== 0) {
      ...
      objectPath.set(
        lDto,
        "FormatedData.CalculatedPremiumDetails. TotalNumberofFish",
        lDto.InsurableItem[0].RiskItems.length
      );
      setDto({ ...lDto });
      // console.log("ddd11111111dd", successRecord, lDto.InsurableItem[0].RiskItems);
      await GenericApi("NepalAgriFish", "NepalFishRatingAPI", lDto).then(async (x) => {
        if (x.finalResult) {
          // console.log("ppx", x);
          // debugger;
          if (x.finalResult.FinalPremium !== "") {
            ...
            x.finalResult.NepalFishBaseRating.output.forEach((x1, i) => {
              // objectPath.set(
              //   lDto,
              //   `FormatedData.CalculatedPremiumDetails.Output.${i}.AgriGeneralRateNew_Age`,
              //   x1.AgriGeneralRateNew_Age
              // );
              objectPath.set(lDto, `InsurableItem.0.RiskItems.${i}.DirectDiscount`, "No");
              objectPath.set(lDto, `InsurableItem.0.RiskItems.${i}.TypeofInsurance`, "Individual");
              objectPath.set(
                lDto,
              ...
              objectPath.set(
                lDto,
                `FormatedData.CalculatedPremiumDetails.Output.${i}.InsuredName`,
                lDto.InsurableItem[0].RiskItems[i].InsuredName
              );

              // if (lDto.TypeofInsurance === "Individual" && excelData === 1) {
              // if (lDto.TypeofInsurance === "Individual" && lDto.Temp.Upload === 1)
              if (
                lDto.InsurableItem[0].TypeofInsurance === "Individual" &&
                ...
                objectPath.set(lDto, `InsurableItem.0.RiskItems.0.TypeofInsurance`, "Group");
                setDto({ ...lDto });
                // swal.fire({
                //   icon: "error",
                //   text: "Individual Not more then one file",
                // });
              }
              ...
              objectPath.set(
                lDto,
                `InsurableItem.0.RiskItems.${i}.PremiumAmount`,
                formater.format(x1.BasePremium)
              );
            });
  ...
  const handleUploadArrData = ({ excelData, ws }) => {
    ...
    console.log("....", res);
    if (lDto.InsurableItem[0].TypeofInsurance === "Individual") {
      // debugger;
      const firstObj = res.successRecord[0];

      lDto.InsurableItem[0].RiskItems[0] = firstObj;
      lDto.Temp.Upload[0] = firstObj;
      setDto({ ...lDto });
    }
    if (SuccessRec.length > 1) {
      // swal.fire({
      //   icon: "error",
      //   text: "Individual Not more then one file",
      // });
      ...
      <br>Incorrect Column Headers</br>`,
        ...
        res.successRecord.forEach((x, i) => {
          // objectPath.set(lDto, `InsurableItem.0.RiskItems.${i}.NumberofFish`, x.NumberofFish);
          setDto({ ...lDto });
          // if (
          ...
          // }
          if (x.NumberofFish === undefined) {
            res.successRecord[i].NumberofFish = x.NumberofFish;
            setSuccessRec([...res.successRecord]);
          }
        });
        ...
        res.failureRecord.forEach((x, i) => {
          ...
          // }
          if (x.NumberOfFish && x.NumberOfFish !== undefined) {
            res.successRecord[i].NumberOfFish = "";
            setFailureRec([...res.failureRecord]);
          }
          ...
          if (x.PremiumAmount && x.PremiumAmount !== undefined) {
            res.failureRecord[i].PremiumAmount = "";
            setFailureRec([...res.failureRecord]);
          }
        });
        // debugger;
  ...
  // lDto.TotalNumberofFish = lDto.InsurableItem[0].RiskItems[index].NumberofFish.length;

  const onUploadArr = (files1) => {
    // debugger;
    const files = files1;
    const fileReader = new FileReader();
    fileReader.readAsArrayBuffer(files.target.files[0]);
    fileReader.onload = (e) => {
      ...
      if (lDto.InsurableItem[0].TypeofInsurance === "Individual" && excelData.length > 1) {
        swal.fire({
          icon: "error",
          text: "Individual Not Accept more then 1 row",
        });
      }
  ...
  useEffect(() => {
    if (SuccessRec1.length === 0 && lDto?.InsurableItem[0]?.RiskItems[0]?.InsuredName === "") {
      setSuccessRec([]);
    }
  }, [SuccessRec.length > 0]);
  ...
  const onSearchFilter = (e) => {
    ...
    const Data = GetData.filter(
      (item) =>
        item.InsuredName.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) > -1 ||
        item.InsuredAddress.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) > -1 ||
        item.NumberofFish.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) > -1 ||
        item.AgeofFish.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) > -1 ||
        item.BreedCasteVariety.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) >
          -1 ||
        item.TypeOfFish.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) > -1 ||
        item.AreaOfPond.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) > -1 ||
        item.AverageWeightOfFish.toString().toUpperCase().indexOf(e.target.value.toUpperCase()) >
          -1 ||
            ...
            columns: [
              {
                ...
                renderCell: (param) => (
                  <MDInput
                    name="NumberOfFish"
                    value={param.row.NumberOfFish}
                    onChange={(e) => handleDataGrid(e, param)}
                    error={flag.NumberofFish === true}
                    helperText={
                      flag.NumberofFish === true ? "Accept only Numbers greater than Zero" : ""
                    }
        ...
        [
          {
            ...
            // path: `InsurableItem.0.RiskItems.0.TypeofInsurance`,
            disableOnReset: true,
          },
          {
            type: "AutoComplete",
          ...
          },
          {
            type: "Button",
            label:
              objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") !== ""
                ? // ||
            ...
            // endIcon:
            //   masters1.flag.backdropflag && lDto.loading ? <CircularProgress size="20px" /> : null,
            spacing: 2,
            variant: "outlined",
            component: "label",
            typeFormat: (
            ...
            disabled:
              lDto.TypeofInsurance === "Individual" && lDto.InsurableItem[0].RiskItems.length === 1,
          },
          {
            type: "Typography",
            label: "",
            spacing: 5,
            visible:
              objectPath.get(dto, "InsurableItem.0.RiskItems.0.InsuredName") === "" &&
              objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") === undefined,
          },
          {
            type: "Custom",
            visible:
          ...
          },
          {
            ...
            visible:
              objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") !== "" &&
              objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") !== undefined,
            // objectPath.get(lDto, "InsurableItem.0.RiskItems.0.Number of Fish") !== undefined,
            // objectPath.get(lDto, "InsurableItem.0.RiskItems.0.Number of Fish") !== undefined ||
            // objectPath.get(lDto, "Temp.Upload.1.Number of Fish") !== undefined,
            spacing: 1,
            variant: "outlined",
            color: "error",
            onClick: () => onClear(),
          },
          {
            ...
            visible:
              ...
              //   objectPath.get(lDto, "NumberofFish") !== "" &&
              //   objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") !== undefined) ||
              objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") !== "" &&
              objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") !== undefined,
            rowId: "row_id",
            columns: [
              ...
              { field: "InsuredAddress", headerName: "Insured Address", width: 200 },
              { field: "NumberofFish", headerName: "Number of Fish", width: 200 },
              { field: "AgeofFish", headerName: "Age of Fish", width: 200 },
              { field: "BreedCasteVariety", headerName: "Breed/Caste/Variety", width: 200 },
              { field: "TypeOfFish", headerName: "Type Of Fish", width: 200 },
              { field: "AreaOfPond", headerName: "Area Of Pond", width: 200 },
              { field: "AverageWeightOfFish", headerName: "Average Weight Of Fish", width: 200 },
          ...
          },
          {
            type: "Custom",
            visible: true,
            // visible: objectPath.get(lDto, "NumberofFish") !== "",
            spacing: 5,
            return: (
              <MDBox
                sx={{
                ...
                p={3}
              >
                <MDTypography variant="h6">Total Number Of Fish</MDTypography>
                {/* <MDTypography variant="h6">
                    {objectPath.get(lDto, "FormatedData.CalculatedPremiumDetails.TotalNumberofFish")}
                  </MDTypography> */}
                <MDTypography variant="h6">
                  {" "}
                  {/* {objectPath.get(lDto, "InsurableItem.0.RiskItems.0.Number of Fish") !== undefined
            ...
            return: (
              ...
              >
                ...
                <MDTypography variant="h6">
                  ...
                  {/* रु{objectPath.get(lDto, "FormatedData.CalculatedPremiumDetails.TotalSumInsured")} */}
                </MDTypography>
              </MDBox>
            ),
          },
...
const getOnNextClick = async ({ activeStep, dto, setBackDropFlag, setDto }) => {
  ...
  const CheckStatus = async () => {
    //  debugger;
    if (objectPath.get(lDto, "InsurableItem.0.RiskItems.0.InsuredName") !== "") {
      await swal
        .fire({
          icon: "warning",
            ...
            if (x.finalResult) {
              if (x.finalResult.FinalPremium !== "") {
                ...
                x.finalResult.NepalFishBaseRating.output.forEach((x1, i) => {
                  ...
                  objectPath.set(
                    lDto,
                    `FormatedData.CalculatedPremiumDetails.Output.${i}.TotalNumberOfFish`,
                    x1.TotalNumberOfFish
                  );
                  objectPath.set(lDto, `Temp.Upload.${i}.Rate(%)`, "3%"); // x1.AgriGeneralRateNew
                  objectPath.set(lDto, `Temp.Upload.${i}.PremiumAmount`, x1.BasePremium);
                  objectPath.set(lDto, `InsurableItem.0.RiskItems.${i}.Rate(%)`, "3%");
                  objectPath.set(
                    lDto,
                    `FormatedData.CalculatedPremiumDetails.Output.${i}.InsuredName`,
                    lDto.InsurableItem[0].RiskItems[i].InsuredName
                    // x1.TotalNumberofFish
                    // lDto.InsurableItem[0].RiskItems[i]["Number of Fish"]
                  );
                  objectPath.set(
                    lDto,
                    `InsurableItem.0.RiskItems.${i}.formatedSumInsured`,
  ...
  switch (activeStep) {
    case 0:
      ...
      // lDto.InsurableItem[0].RiskItems[0].NumberOfFish = lDto.NumberOfFish;
      // setDto({ ...lDto });
      lDto.InsurableItem[0].RiskItems[0].SumInsured = lDto.Temp.DataGridValues[0].SumInsured;
      lDto.InsurableItem[0].RiskItems[0].NumberOfFish = lDto.Temp.DataGridValues[0].NumberOfFish;
      setDto({ ...lDto });
      if (
        lDto.Temp.DataGridValues[0].NumberOfFish !== "" &&
        lDto.Temp.DataGridValues[0].SumInsured !== ""
      ) {
        fun = await GenericApi("NepalAgriFish", "NepalFishRatingAPI", lDto).then(async (x) => {
          // debugger;
          console.log("x1", x);
          if (x.finalResult) {
            // lDto.ProposerDetails = { ...lDto.ProposerDetails[0] };
            ...
            objectPath.set(
              lDto,
              "Temp.DataGridValues.0.Premium Amount",
              formater.format(x.finalResult.NepalFishBaseRating.output[0].BasePremium)
            );
            objectPath.set(
              lDto,
      ...
      if (
        lDto.InsurableItem[0].RiskItems[0].NumberOfFish === "" ||
    ...
    case 1:
      ...
      // break;
      if (
        lDto?.Temp?.Upload?.length !== lDto.InsurableItem[0].RiskItems.length &&
      ...
      if (
        lDto?.Temp?.Upload[0]?.InsuredName !== undefined &&
        lDto?.Temp?.Upload?.length < lDto.InsurableItem[0].RiskItems.length
      ) {
        lDto.InsurableItem[0].RiskItems = lDto.Temp.Upload;
        setDto({ ...lDto });
      ...
      if (lDto.proposalNo === undefined) {
        ...
        fun = await SaveCreateProposal(lDto).then(async (x) => {
          ...
          if (x.data.proposalNumber) {
            lDto.ProposerDetails = { ...lDto.ProposerDetails[0] };
            await GetProductByCode("NepalAgriFish").then(async (x2) => {
...
const getButtonDetails = ({ activeStep }) => {
  ...
  const onReset2 = (dto, setDto, dispatch) => {
    ...
    objectPath.set(lDto, "Temp.Upload", []);
    lDto.InsurableItem[0].RiskItems = [{ ...RiskItemFish() }];
    delete lDto.InsurableItem[0].TypeofInsurance;
    lDto.TotalSumInsured = "";
    lDto.NumberofFish = "";
    setDto({ ...lDto });
    // lDto.InsuredName = "";
