...
import {
  ...
  BranchDetails,
  RollOverDocuments,
} from "./data/Json/HomeInsuranceJson";
...
const getSectionContent = ({ activeStep, dto, setDto, masters, setMasters }) => {
  ...
  const OnQuoteTotalSumInsuredVal = (e) => {
    if (e.target.value !== "") {
  ...
  const onBlurTransliteration = async (e, a, EF, ET, i) => {
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      if (varName === "IndividualPermanentAddressEnglish") {
        lDto.ProposerDetails[i].PermanentAdrress.PermanentAdrressNepali = Text;
      }
    ...
    dto.ProposerDetails.forEach((x, i) => {
      arr.push([
        ...
        },
        {
          ...
          visible: true,
          path: `ProposerDetails.${i}.PermanentAdrress.PermanentAdrressEnglish`,
          onChangeFuncs: [IsFreetextNoSpace],
        ...
        },
        {
          ...
          visible: true,
          path: `ProposerDetails.${i}.PermanentAdrress.PermanentAdrressNepali`,
          disabled: true,
          accordionId: 1,
          spacing: 3,
        },
      ...
      data = [
        ...
        [
          ...
          },
          {
            ...
            customOnBlur: (e) => OnQuoteTotalSumInsuredVal(e),
          },
          {
            type: "Typography",
            visible: true,
        ...
        [
          {
            ...
            // options: masters.ConstructionofBuilding,
          },
          {
            type: "Input",
  ...
  if (lDto !== null) {
    switch (activeStep) {
      case 0:
        lDto.TotalSumInsured = lDto.ListofProperty.TotalSumInsured;
        setDto({ ...lDto });
        fun = await GenericApi("NepalHomeInsurance", "NepalHomeProductAPI", lDto).then(
          async (x) => {
            if (x.finalResult) {
        ...
        if (lDto.proposalNo === undefined) {
          ...
          await SaveCreateProposal(lDto).then(async (x) => {
            ...
            if (x.data.proposalNumber) {
              await GetProductByCode("NepalHomeInsurance").then(async (x2) => {
                lDto.ProposerDetails = { ...lDto.ProposerDetails[0] };
