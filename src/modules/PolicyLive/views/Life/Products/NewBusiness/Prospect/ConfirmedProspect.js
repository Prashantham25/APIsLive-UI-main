...
function ConfirmedProspect({ styles, setLoading, setPage, setSelectedId }) {
  ...
  useEffect(async () => {
    setLoading(true);
    await GetLeadPool("Prospect").then((res) => {
      setLoading(false);
  ...
  return (
    <MDBox sx={{ ...centerRowStyle, flexDirection: "column", width: "100%" }}>
      <MDTypography sx={headingStyle}>Confirmed Prospect</MDTypography>
...
export default ConfirmedProspect;
