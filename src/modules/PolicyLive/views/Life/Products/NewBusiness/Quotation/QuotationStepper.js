...
function QuotationDetailsModal({ dto, verticalTabMap, setLoading, productIndex }) {
  ...
  const sendMail = () => {
    ...
    const { ProductCode } = dto.productDetails[productIndex];
    GenericApi(ProductCode, "LICQuotationMailApi", {
      ...productDetails,
...
function QuoteSummary({ dto, productIndex, verticalTabMap, setLoading }) {
  ...
  const [agentChannelDetails, setAgentChannelDetails] = useState({ ...channelDetails });
  const [requestJson, setRequestJson] = useState({
    merchantid: agentChannelDetails.merchantid,
    validuser: "",
    agency_code: "",
    agency_type: "",
    agent_name: "",
  ...
  const handleOnBlurCall = async () => {
    if (requestJson.merchantid !== agentChannelDetails.merchantid) {
      setLoading(true);
      const response = await GenericApi("LifeInsurance", "LICLoginAPI", requestJson);
      setLoading(false);
      const newAgentDetails = response.finalResult;
      if (
        newAgentDetails.validuser === true &&
        ((newAgentDetails.supervisor_type === "DO" &&
          newAgentDetails.supervisor_code === agentChannelDetails.supervisor_code &&
          newAgentDetails.dev_off_type === agentChannelDetails.dev_off_type) ||
          (newAgentDetails.supervisor_type !== "DO" &&
            newAgentDetails.supervisor_code === agentChannelDetails.supervisor_code))
      ) {
        setAgentChannelDetails({ ...newAgentDetails });
        setUpdateButton(true);
      } else {
        setSnackFlag({
          message:
            newAgentDetails.validuser === true
              ? "Agent not supervised by you"
              : "No matching agent found",
          status: "error",
        });
        setRequestJson({ ...requestJson, merchantid: agentChannelDetails.merchantid });
      }
    }
  };
  const handleUpdateChannelDetails = async () => {
    ...
    const responses = await Promise.all([
      ...dto.productDetails.map((elem) => {
        const temp = {
          ...elem,
        };
        delete temp.dynamicContent;
        delete temp.quoteSummary;

        let QuoteNo = null;
        if (!checkForValue(temp.QuoteNo)) QuoteNo = temp.QuoteNo;

        const newDetails = {
          ...dto,
          ProposerDetails: { ...dto.ProposerDetails },
          ChannelDetails: { ...agentChannelDetails, ChannelType: "A2C" },
        };
        ...
        temp.InsurableItem[0].RiskItems = [
          ...temp.InsurableItem[0].RiskItems.map((x) => ({
            ...x,
        ...
        const detailsToSave = {
          ...
          "Product Id": temp.ProductId,
          ProductCode: temp.ProductCode,
          QuoteNo,
        };

        return SaveQuotation(detailsToSave);
      }),
    ]);
    responses.forEach((x) => {
      if (x.status > 3) {
    ...
    if (flag === true) {
      setSnackFlag({ message: "Channel Details updated successfully!", status: "success" });
      setUpdateButton(false);
    } else {
      setSnackFlag({ message: "Update Failed. Try Again!", status: "error" });
    }
    setLoading(false);
  };
                ...
                <Grid container display="flex" spacing={1.5}>
                  <Grid item xs={12} sm={12} md={6} lg={6} xl={6} xxl={6}>
                    <MDInput
                      ...
                      label="Agency Code"
                      disabled={agentChannelDetails.supervisor !== true}
                    />
                  </Grid>
                  <Grid item xs={12} sm={12} md={6} lg={6} xl={6} xxl={6}>
                    <MDInput value={agentChannelDetails.dev_off_code} label="DO Code" disabled />
                  </Grid>
                  <Grid item xs={12} sm={12} md={6} lg={6} xl={6} xxl={6}>
                    <MDInput
                      value={agentChannelDetails.ag_branch}
                  ...
                  <Grid item xs={12} sm={12} md={6} lg={6} xl={6} xxl={6}>
                    <MDInput value={agentChannelDetails.agent_name} label="Name" disabled />
                  </Grid>
                  <Grid item xs={12} sm={12} md={6} lg={6} xl={6} xxl={6}>
                    <MDInput
                      value={agentChannelDetails.ag_mobileno}
                      label="Mobile Number"
                      disabled
                    />
                  </Grid>
                  <Grid item xs={12} sm={12} md={6} lg={6} xl={6} xxl={6}>
                    <MDInput value={agentChannelDetails.ag_emailid} label="Email ID" disabled />
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                    <MDBox sx={{ width: "100%", justifyContent: "end", display: "flex" }}>
                      {updateButton && (
                        <MDButton onClick={handleUpdateChannelDetails}>Update Details</MDButton>
                      )}
                    </MDBox>
                  </Grid>
                </Grid>
  ...
  */
    ...
    const quotationSave = async (index) => {
      ...
      const newDetails = {
        ...dto,
        ProposerDetails: { ...dto.ProposerDetails },
        ChannelDetails: { ...channelDetails, ChannelType: "A2C" },
      };
      delete newDetails.productDetails;
      ...
      setLoading(false);
      setSnackFlag({ message: "Error occured in save", status: "error" });
      // swal({
      //   text: res.responseMessage,
      //   icon: "error",
      // });
      ...
      if (lActiveStep < 2) {
        ...
        const getPlanMasters = async (productCode, productIndex) => {
          ...
          await Promise.all([QueryExecution(requestJson)]).then((res1) => {
            ...
            if (requestBody.InsurableItem[0].RiskItems[0].Benefit === undefined) {
            ...
            } else newDetails.InsurableItem = requestBody.InsurableItem;
    ...
    useEffect(() => {
      if (!checkForValue(dto.ProposerDetails.opportunityId))
        SaveOpportunity({
          ...
          [
            ...
            },
            {
              ...
              customOnChange: (e, a, setErrorFlag, setErrorText) => {
                ...
                const newMasters = { ...masters };
                dto.productDetails.forEach((x, index) => {
                  newMasters[`Plan-${index}`] = [];
                });
                setMasters(newMasters);
              },
            },
            {
              ...
              options: getMaster("Gender"),
              customOnChange: (e, a) => {
                assignValueId(a, "ProposerDetails", "Gender");
                const newMasters = { ...masters };
                dto.productDetails.forEach((x, index) => {
                  newMasters[`Plan-${index}`] = [];
                });
      ...
      case 1:
        data = [
          [
            ...
            {
              type: "CustomTabs",
              visible: Object.keys(verticalTabMap || {}).length > 0,
            ...
            },
            {
              ...
              variant: "outlined",
              startIcon: <Add />,
              visible: Object.keys(verticalTabMap || {}).length < 5,
              onClick: addProduct,
              spacing: 2,
              sx: {
                borderRadius: "0.25rem",
...
const getOnNextClick = async ({
  ...
  switch (lActiveStep) {
    ...
    case 0:
      fun = true;
      let newProposerDetails = { ...dto.ProposerDetails };
      if (checkForValue(dto.ProposerDetails.contactId)) {
        const saveJson = {
          ...dto.ProposerDetails,
          contactId: 0,
          opportunityId: 0,
        ...
        setLoading(true);
        const res = await SaveContact(saveJson);
        setLoading(false);
      ...
      setDto({
        ...dto,
        ProposerDetails: { ...newProposerDetails },
    ...
    case 5:
      ...
      if (channelDetails.ChannelType === "BANCA") {
        navigate(
          `/Proposal/BancaProposalCkycDetails?OpportunityId=${dto.ProposerDetails.opportunityId}`
        );
      } else {
...
const getMasterData = async ({
  ...
  if (!checkForValue(selectedId)) {
    ...
    if (opportunityData.AdditionalDetailsJson?.AutoSave)
      setDto(opportunityData.AdditionalDetailsJson.AutoSave);
