...
import "../../NewBusiness/Quotation/Checkbox.css";

import Plans from "../../NewBusiness/Quotation/Plans";
...
import ReqDocCard from "../data/ReqDocCard";
import FundOptionContent from "../data/FundOptionContent";
// import { GetProdPartnermasterDataCN } from "../data";
...
const getDecimalAge = (date, currentDate) => {
  const currDate = currentDate ? new Date(currentDate) : new Date();
  const birthdate = new Date(date);
  const Age = GenericAgeCalculator(birthdate, currDate, "combined");
  // const diff = currDate.getTime() - birthdate.getTime();
  // const Age = diff / (1000 * 60 * 60 * 24 * 365.25);
  return Age.yearAge + Age.monthAge / 12;
};
...
function QuotationDetailsModal({ dto, verticalTabMap, setLoading, productIndex }) {
  ...
  const sendMail = () => {
    ...
    const { ProductCode } = dto.productDetails[productIndex];
    GenericApi(ProductCode, "LICQuotationMailApi", productDetails);
            ...
            <Grid container spacing={1}>
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                {Object.keys(verticalTabMap || {}).map((elem) => (
                  <Grid container spacing={1} sx={{ pt: "1rem" }}>
                    <Grid item xs={8} sm={8} md={8} lg={8} xl={8} xxl={8}>
                      <MDTypography variant="body1" sx={{ fontSize: "0.875rem", color: "#5F5F5F" }}>
...
function QuotationSummary({ dto, productIndex, setActiveStep }) {
  console.log("Illustration Button", dto);
  // const [controller] = useDataController();
  ...
  console.log("1234567890", IllustrationButton);
  ...
  const getSummaryField = (field) => {
    ...
    switch (type) {
      ...
      case "Policy Term":
        return { label: "Policy Term", value: `${productDetails.PolicyTerm} Years` };
      case "Paying Term":
        return { label: "Premium Paying Term", value: `${productDetails.PremiumPayingTerm} Years` };
      case "Payment Mode":
        return { label: "Premium Payment Mode", value: productDetails.PreferredMode };
      case "Residential Status":
        return {
          label: "Residential Status",
          value: dto?.ProposerDetails?.ResidentStatus,
        };
      case "Date of Birth":
        return {
          label: "Date of Birth",
  ...
  return (
    <MDBox
      sx={{ width: "100%", pt: "1rem", background: "rgba(245, 245, 245, 1)", p: 2, mt: "1.5rem" }}
    >
            ...
            <Grid container spacing={1}>
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                <Stack direction="row" justifyContent="space-between">
                  <MDTypography color="primary" sx={{ fontWeight: "700", fontSize: "18px" }}>
  ...
  dto.productDetails.forEach((elem, index) => {
    if (index === productIndex) {
      if (!checkForValue(elem.PremiumDetails)) {
        ...
        termRiderPrem += getInt(elem.PremiumDetails["Term Rider Premium"]);
        basePrem += getInt(elem.PremiumDetails["Base Plan Premium"]);
        PWPrem += getInt(elem.PremiumDetails["Premium Waiver Premium"]);
      }
    }
  });
  // const getValue = (value) => (value !== undefined && value !== null && value !== "" ? value : "");
      ...
      <MDBox sx={{ width: "100%", display: "flex", justifyContent: "center", textAlign: "start" }}>
        {!checkForValue(productDetails.PremiumDetails["Total Premium"]) && (
          <MDBox sx={{ width: "100%", mt: "1.5rem" }}>
            {/* {cardRef !== null && premiumModes && premiumModes[premiumModeTab].length > 1 && (
              <Card
                sx={{
                  ...movingCardPosition,
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={12} md={7} lg={7} xl={7} xxl={7}>
                <Grid container spacing={2} p={2}>
                  {false && (
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                      <MDTypography sx={{ bgcolor: "red", fontSize: "1rem", color: "#ffffff" }}>
                        Please review and save the quote to proceed to Proposal
                      </MDTypography>
                    </Grid>
                  )}
                  {premiumModes.length > 1 &&
                    premiumModes.map((x, index) => (
                      <Grid item xs={12} sm={12} md={6} lg={4} xl={4} xxl={4}>
                        <Card
                          ...
                          sx={{
                            ...
                            border:
                              ...
                              (premiumModes[1] &&
                                premiumModes[1]["Premium Mode"] !== "12" &&
                                index === 0)
                        ...
                        >
                          <Stack direction="row" spacing={2}>
                            ...
                            <MDTypography
                              ...
                              sx={{
                                ...
                                fontWeight: 600,
                                color:
                                  premiumModeTab === index
                                    ? "#ffffff"
                                    : ColorsSetting().primary.main,
                                alignSelf: "center",
                              }}
                            >
                          ...
                          {productDetails.PlanNumber !== "849" &&
                            productDetails.PlanNumber !== "852" &&
                            productDetails.PlanNumber !== "867" &&
                            productDetails.PlanNumber !== "935" && (
                              <MDTypography
                                className="text"
                                sx={{
                                  color: premiumModeTab === index ? "#ffffff" : "#000000",
              ...
              <Grid item xs={12} sm={12} md={5} lg={5} xl={5} xxl={5}>
                ...
                <MDBox
                  ...
                  sx={{
                    ...
                    top: matches ? "15px" : 0,
                    right: matches ? "15px" : 0,
                    maxWidth: matches ? "40%" : "auto",
                  }}
                  ref={componentRef4}
                >
                  <MDTypography sx={{ fontSize: "1rem", color: "#000000", fontWeight: 500 }}>
                  ...
                  <Grid container spacing={1}>
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                      <Grid container spacing={1} sx={{ pt: "1rem" }}>
                        ...
                        {!checkForValue(abdbPrem) && abdbPrem !== 0 && (
                          <>
                            <Grid item xs={8} sm={8} md={8} lg={8} xl={8} xxl={8}>
                              <MDTypography
                                variant="body1"
                        ...
                        {!checkForValue(cirPrem) && cirPrem !== 0 && (
                          <>
                            <Grid item xs={8} sm={8} md={8} lg={8} xl={8} xxl={8}>
                              <MDTypography
                                variant="body1"
                        ...
                        {!checkForValue(termRiderPrem) && termRiderPrem !== 0 && (
                          <>
                            <Grid item xs={8} sm={8} md={8} lg={8} xl={8} xxl={8}>
                              <MDTypography
                                variant="body1"
                        ...
                        {!checkForValue(PWPrem) && PWPrem !== 0 && (
                          <>
                            ...
                            <Grid item xs={4} sm={4} md={4} lg={4} xl={4} xxl={4}>
                              ...
                              >
                                {currencySymbol}
                                {PWPrem !== 0 ? formatCurrency(PWPrem) : 0}
                              </MDTypography>
                            </Grid>
                          </>
                        )}
                        ...
                        <Grid item xs={8} sm={8} md={8} lg={8} xl={8} xxl={8}>
                          <MDTypography
                            ...
                            sx={{ fontSize: "0.875rem", color: "#5F5F5F", fontWeight: 500 }}
                          >
                            GST({dto.productDetails[productIndex].PremiumDetails["GST Rate"]}%)
                          </MDTypography>
                        </Grid>
                        <Grid item xs={4} sm={4} md={4} lg={4} xl={4} xxl={4}>
                          <MDTypography
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                      <Stack direction={matches ? "row" : "column"} justifyContent="space-between">
                        {IllustrationButton?.length > 0 && (
                          <MDButton
                            sx={{ mb: matches ? 0 : "10px" }}
                            startIcon={<Download />}
                            onClick={onBenifitIllustration}
                          >
                            Benefit Illustration
                          </MDButton>
                        )}
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                      ...
                      >
                        Note : Calculated premium may vary further, according to the health and
                        habits selected in the online proposal form.
                      </MDTypography>
                    </Grid>
                  </Grid>
                </MDBox>
...
const getSectionContent = ({
  ...
  let data = [];
  // let lActiveStep = activeStep;
  if (dto?.ProposerDetails?.IsProposerSameAsInsured === "Yes") {
  ...
  */
    ...
    const { centerRowStyle } = styles;
    const matches = useMediaQuery("(min-width:600px)");
    const mobileView = useMediaQuery("(min-width:992px)");
          ...
          RiskItems: [
            {
              ...
              AgeNBD: dto.ProposerDetails.AgeNBD,
              PassportNo: "",
              PreExistingDisease: "",
              SumAssured: "",
              RiderDetails: [
    ...
    const getAge = (basePath, value, setErrorFlag, setErrorText, startDate) => {
      const birthdate = new Date(value);
      // const startDate =
      //   productIndex === 0 || !checkForValue(productIndex)
      //     ? new Date(dto.productDetails[productIndex].DateOfCommencement)
      //     : new Date();
      const ageObj = GenericAgeCalculator(birthdate, startDate, "combined");
      const Age = ageObj.yearAge;
      const AgeNBD = Math.round(ageObj.yearAge + ageObj.monthAge / 12);
      setErrorFlag(false);
      setErrorText("");
      if (basePath === "") setDto({ ...dto, DOB: value, Age, AgeNBD });
      else {
        const dummy = get(dto, basePath);
        set(dto, basePath, { ...dummy, DOB: value, Age, AgeNBD }, setDto);
      }
    };
    ...
    const getSpouseContent = (index) => [
      ...
      },
      {
        ...
        maxDate: new Date(new Date() - 86400000 * 90),
    ...
    const quotationSave = async (index) => {
      ...
      delete productDetails.planDetailsJson;
      ...
      const newDetails = {
        ...
        ChannelDetails: {
          ...
          ...channelDetails,
          ChannelType: "D2C",
          BranchCode: dto.ProposerDetails.PermanentAddress.BranchCode,
        },
      };
      ...
      const detailsToSave = {
        "Agent Id": quotationDetail !== undefined ? quotationDetail["Agent Id"] : 0,
        QuotationDetails: {
          ...newDetails,
          ...productDetails,
        },
      ...
      if (res.status <= 3) {
        ...
        const { ProductCode } = dto.productDetails[index];

        await GenericApi(ProductCode, "LICBenifitIllustrationAPI", productDetails).then(
          async (res) => {
            if (res.status === 1) {
              const req = {
                fileUploadDTOs: [
                  {
                    tagName: "LIC",
                    fileName: `${dto.ProposerDetails.opportunityId}/${productDetails.PlanNumber}_BenefitIllustration.pdf`,
                    fileExtension: "PDF",
                    fileData: res.finalResult.FinalResult,
              ...
              await DocumentSimpleupload(req);
            }
          }
        );
        ...
        EventCommunicationExecution(req);

        let DocumentDetails = [
          {
            DocumentType: "Quotation PDF",
            DocumentId: `${res.quotation.quoteNo}.pdf`,
            DocumentName: `${res.quotation.quoteNo}.pdf`,
            ContentType: "application/pdf",
          },
          {
            DocumentType: "Sales Brouchure",
            DocumentId: `Plans/${productDetails.PlanNumber}/${productDetails.PlanNumber}Brochure.pdf`,
            DocumentName: `Plans/${productDetails.PlanNumber}/${productDetails.PlanNumber}Brochure.pdf`,
            ContentType: "application/pdf",
          },
        ];
        const IllustrationButton = dto.productDetails[0].dynamicContent.filter(
          (x) => x.type === "IllustrationButton"
        );
        if (IllustrationButton.length > 0) {
          DocumentDetails = [
            ...DocumentDetails,
            {
              DocumentType: "Benefit Illustration",
              DocumentId: `${dto.ProposerDetails.opportunityId}/${productDetails.PlanNumber}_BenefitIllustration.pdf`,
              DocumentName: `${dto.ProposerDetails.opportunityId}/${productDetails.PlanNumber}_BenefitIllustration.pdf`,
              ContentType: "application/pdf",
            },
          ];
        }
        ...
        const req1 = {
          TxnId: res.quotation.quotationId,
          TxnType: "Quotation",
          TxnNo: res.quotation.quoteNo,
          DocumentDetails: {
            DocumentDetails,
          },
        };
        ...
        SaveUploadDocumentDetails(req1);

        setTimer(0);
        setTimeout(() => {
          // setTimer(timer + 1);
          GenericApi("LifeInsurance", "SendQuoteMailAPi", {
            OppurtunityID: dto.ProposerDetails.opportunityId,
            EmailID: dto.ProposerDetails.EmailId,
            ContactNo: dto.ProposerDetails.ContactNo,
            MasterType: "QuotePDF",
          });
        }, 1000 * 4);
      ...
      if (dto.productDetails[0]?.QuoteNo) {
        GenericApi("LifeInsurance", "SendQuoteMailAPi", {
          OppurtunityID: dto.ProposerDetails.opportunityId,
          EmailID: dto.ProposerDetails.EmailId,
          ContactNo: dto.ProposerDetails.ContactNo,
      ...
      setLoading(false);
      setSnackFlag({ message: "Error in quotation save", status: "error" });
    ...
    const validateAge = (productIndex) => {
      ...
      elem.InsurableItem[0].RiskItems.every((x) => {
        const decimalAge = getDecimalAge(x.DOB, elem.DateOfCommencement);
        if (decimalAge < minAge || decimalAge > maxAge) {
          flag = false;
          return false;
        }
        return true;
    ...
    const memberContent = (productIndex, detailsJson, variables, functions, skipSelf) => {
      let content = [];
      ...
      dto.productDetails[productIndex]?.InsurableItem[0].RiskItems.forEach((foo, index) => {
        if (skipSelf !== true || (skipSelf === true && index !== 0)) {
          const x = [
            {
              type: "Typography",
              label: index > 0 ? `Life Assured ${index + 1} Details` : `Life Assured Details`,
              visible: true,
              spacing:
            ...
            { type: "Img", visible: mobileView === true, src: nameImg, spacing: 1, sx: imgSX },
            {
              ...
              options:
                // dto?.ProposerDetails?.IsProposerSameAsInsured !== "Yes"
                //   ? getMaster("relation").filter((x) => x.mValue !== "Self")
                //   :
                skipSelf === true
                  ? getMaster("relation").filter((x) => x.mValue !== "Self")
                  : getMaster("relation"),
            ...
            },
            {
                    ...
                    },
                    {
                      ...
                      value:
                        getAutocompleteValue(
                          `Plan-${productIndex}`,
                          dto.productDetails[productIndex].PlanId
                        )?.additionalDetailsJson?.Limits?.premiumTermAge -
                        AgeCalculator(new Date(a)),
                    },
                  ]);
                }
                getAge(
                  ...
                  setErrorFlag,
                  setErrorText,
                  new Date(variables.elem.DateOfCommencement)
                );
                if (
                  getAutocompleteValue(
                    `Plan-${productIndex}`,
                    dto.productDetails[productIndex].PlanId
                  )?.additionalDetailsJson?.CustomAgeOnChange
                ) {
                  const ageObj = GenericAgeCalculator(
                    new Date(a),
                    new Date(variables.elem.DateOfCommencement),
                    "combined"
                  );
              ...
              errorFlag:
                ageResponse !== true &&
                (ageResponse.minAge > getDecimalAge(foo.DOB, variables.elem.DateOfCommencement) ||
                  ageResponse.maxAge < getDecimalAge(foo.DOB, variables.elem.DateOfCommencement)),
              errorText:
                ageResponse !== true &&
                (ageResponse.minAge > getDecimalAge(foo.DOB, variables.elem.DateOfCommencement) ||
                  ageResponse.maxAge < getDecimalAge(foo.DOB, variables.elem.DateOfCommencement))
            ...
            },
            {
              type: "Img",
              visible:
                dto.productDetails[productIndex].InsurableItem[0].RiskItems[index].Age < 18 &&
                mobileView === true,
              src: detailsImg,
              spacing: 1,
              sx: imgSX,
    ...
    const getMemberContent = (productIndex, detailsJson, variables, functions, skipSelf) => [
      {
        type: "Custom",
        return: <MDBox />,
        visible: true,
      ...
      ...memberContent(productIndex, detailsJson, variables, functions, skipSelf),
    ];
    ...
    const clearProductDetails = () => {
      const newProductDetails = dto.productDetails.map((newDetails) => ({
        ...prod,
        Product: newDetails.Product,
        Plan: newDetails.Plan,
        ProductId: newDetails.ProductId,
        ProductCode: newDetails.ProductCode,
        PlanNumber: newDetails.PlanNumber,
        PlanCode: newDetails.PlanCode,
        dynamicContent: newDetails.dynamicContent,
        quoteSummary: newDetails.quoteSummary,
        planDetailsJson: newDetails.planDetailsJson,
      }));
      set(dto, `productDetails`, newProductDetails, setDto);
      setCallProductOnLoadFuncs(0);
    };
      ...
      switch (json.type) {
            ...
            customOnChange: (e, a) => {
              setMultipleValues(
                [
                  ...
                  {
                    path: `productDetails.${variables.index}.NachId`,
                    value:
                      a.mValue === "Monthly"
                        ? "6"
                        : a.mValue === "Single"
                        ? "7"
                        : variables.elem.NachId,
                  },
                ],
                variables,
                functions
        ...
        case "PolicePersonnel":
          return {
            ...
            type: "AutoComplete",
            visible:
              variables.visible && variables.elem.InsurableItem[0].RiskItems[memberId].Age >= 18,
        ...
        case "PremiumWaiverBenefit":
          return {
            ...
            visible:
              variables.visible &&
              dto.productDetails[variables.index].InsurableItem[0].RiskItems[memberId].Age < 18 &&
              dto.productDetails[variables.index].InsurableItem[0].RiskItems[memberId].DOB !== "",
            required: true,
            options: getMaster("PremiumWaiver"),
            customOnChange: async (e, a) => {
              setMultipleValues(
        ...
        case "Nach":
          return {
            ...
            visible:
              // variables.visible ||
              (dto.productDetails[variables.index].PolicyType === "ORD" &&
                json.isSSS === true &&
                variables.elem.PreferredMode !== "Single") ||
              !json.isSSS,
            required: true,
        ...
        case "Date Commencement":
          return {
            ...
            customOnChange: (e, a) => {
              const newRiskItems = variables.elem.InsurableItem[0].RiskItems.map((x) => {
                const ageObj = GenericAgeCalculator(new Date(x.DOB), new Date(a), "combined");
                const Age = ageObj.yearAge;
                const AgeNBD = Math.round(ageObj.yearAge + ageObj.monthAge / 12);
                return { ...x, Age, AgeNBD };
              });
              setMultipleValues([
                { path: `productDetails.${variables.index}.DateOfCommencement`, value: a },
                {
                  path: `productDetails.${variables.index}.InsurableItem.0.RiskItems`,
                  value: newRiskItems,
                },
              ]);
            },
                ...
                content =
                  y.type === "MembersContent"
                    ? [
                        ...
                        ...getMemberContent(index, y, variables, functions, true).map((z) => ({
                          ...z,
                          visible: z.visible && visible,
                          accordionId: 1,
                        })),
                    ...
                    ? [
                        ...
                        ...[
                          ...
                          }),
                          {
                            ...
                            subType: "Button",
                            spacing: 12,
                            visible: variables.visible && variables.hasMultipleProducts && false,
                            label: "Calculate Premium",
                            justifyContent: "end",
                            onClick: (flag) => {
                              if (flag === true) newCalculatePremium(variables.index);
      ...
      try {
        ...
        productDetailsContent = [
          ...
          },
          {
            ...
            spacing: 12,
            accordionList: [
              { visible: true, label: "", accordionId: 2 },
              { visible: true, label: "", accordionId: 1 },
              {
                visible:
                  visible &&
    ...
    useEffect(() => {
      if (activeStep < 3) {
        ...
        if (activeStep < 1) {
          Promise.all([
            GetProdPartnerMasterData("LICCountry", {}),
            ...
            GetProdPartnerMasterData("LifeCityMaseter", {}),
          ]).then((res) => {
            setMasters((prevState) => ({
              ...prevState,
              Country: res[0],
            ...
            InsurableItem: [
              {
                ...
                RiskItems: [
                  ...
                  ...productDetails[productIndex].InsurableItem[0].RiskItems.map((x) => ({
                    ...
                    IsProposerSameAsInsured: x.IsProposerSameAsInsured || "Yes",
                  })),
                ],
              },
            ],
    ...
    }, [dto.productDetails.length, activeStep]);
    ...
    useEffect(() => {
      clearProductDetails();
    }, [dto.ProposerDetails?.DOB, dto.ProposerDetails?.Gender]);
    ...
    useEffect(() => {
      SaveOpportunity({
        opportunityId: dto.ProposerDetails.opportunityId,
        needAnalysisJson: null,
        stageId: 3,
        stageStatusId: 1,
        txnType: "",
        ...
        AutoSave: {
          ...
          // QuotationList: dto?.QuotationList,
          // ProposerSameAsInsurable: dto.ProposerSameAsInsurable,
          // ChannelDetails: dto?.ChannelDetails,
          ProposerDetails: dto.ProposerDetails,
          productDetails: dto.productDetails,
          ChannelType: "D2C",
          // isProposerStage: true,
          // ProposalList: dto.ProposalList,
          ProposerContactNo: dto.ProposerDetails.ContactNo,
          // Category: dto.Category,
          // MHRAllocation: dto.MHRAllocation,
        },
      });
    }, [timer]);

    useEffect(async () => {
      if (dto.productDetails && dto.productDetails[0].ProductId) {
        setLoading(true);
        const productAdditionalDetails = await GetProdPartnerMasterData("ProductDetails", {
          parentID: dto.productDetails[0].ProductId,
        });
        const planDetails = await GetProdPartnerMasterData("Plan", {
          parentID: dto.productDetails[0].ProductId,
        });
        setLoading(false);
        const newProductDetails = [...dto.productDetails];
        if (
          ...
          productAdditionalDetails[0].AdditionDetailsJson &&
          productAdditionalDetails[0].AdditionDetailsJson.productControls
        ) {
          newProductDetails[0].dynamicContent = [
            ...productAdditionalDetails[0].AdditionDetailsJson.productControls,
          ];
          newProductDetails[0].quoteSummary = [
            ...productAdditionalDetails[0].AdditionDetailsJson.quoteSummary,
          ];
        }
        if (planDetails && planDetails[0].additionalDetailsJson) {
          newProductDetails[0].planDetailsJson = {
            ...planDetails[0].additionalDetailsJson,
          };
        }
        set(dto, "productDetails", newProductDetails, setDto);
      }
          ...
          [
            {
              ...
              visible: true,
              spacing: 12,
              return: <ReqDocCard mobileView={mobileView} />,
            },
          ],
        ];
        break;
      ...
      case 2:
        data = [
          [
            ...
            {
              type: "CustomTabs",
              visible: Object.keys(verticalTabMap || {}).length > 0 && false,
            ...
            },
            {
              ...
              variant: "outlined",
              startIcon: <Add />,
              visible: Object.keys(verticalTabMap || {}).length < 5 && false,
              onClick: addProduct,
              spacing: 2,
              sx: {
                borderRadius: "0.25rem",
            ...
            ...productDetailsContent,
          ].filter((x) => x?.type === "Accordion" || x?.type === "Split" || x?.accordionId === 2),
          // .map((item) => {
          //   const { splitId, ...newItem } = item;
          //   return newItem;
          // }),
        ];
        break;
      case 3:
        data = [
          [
            ...
            {
              type: "Typography",
              label: "Broad Investment Pattern of the Investible Funds",
              visible: productDetailsContent.some((x) => x?.label === "Fund Option"),
              spacing: 12,
              sx: { fontSize: "1rem", fontWeight: 600 },
              accordionId: 1,
            },
            FundOptionContent(productDetailsContent, productDetails[0]?.ProductId),
          ].filter((x) => x?.type === "Accordion" || x?.type === "Split" || x?.accordionId === 1),
          // .map((item) => {
          //   const { splitId, ...newItem } = item;
          //   return newItem;
          // }),
      ...
      case 4:
        data = [
          ...
          [
            ...
            !checkForValue(productDetailsContent) && productDetailsContent,
            ...productDetailsContent,
          ].filter((x) => x?.type === "Accordion" || x?.type === "Split" || x?.accordionId === 3),
          // .map((item) => {
...
const getOnNextClick = async ({
  ...
  if (dto?.productDetails[0]?.InsurableItem[0]?.RiskItems[0]?.Benefit?.length <= 0) {
    if (activeStep === 3) {
      setActiveStep(5);
      lActiveStep = 4;
    }
  ...
  // if (dto?.ProposerDetails?.IsProposerSameAsInsured === "Yes") {
  //   if (lActiveStep === 1) lActiveStep = 2;
  // }
  ...
  switch (lActiveStep) {
    ...
    case 1:
      // if (masters.declaration.Check1 !== true || masters.declaration.Check2 !== true) {
      //   fun = false;
      //   swal({ icon: "error", text: "Check the declarations" });
      // } else {
      {
        const saveJson = {
          ...dto.ProposerDetails,
        ...
        setLoading(true);
        if (!dto.ProposerDetails?.opportunityId) {
          await SaveContact(saveJson).then((res) => {
            const lDto = dto;
            lDto.ProposerDetails.opportunityId = res.finalResult.opportunityId;
            setDto({ ...lDto });
          });
        }
        setLoading(false);
        ...
        if (dto?.ProposerDetails?.IsProposerSameAsInsured === "Yes") {
                    ...
                    RiskItems: [
                      {
                        ...
                        Name: `${
                          dto?.ProposerDetails?.FirstName
                            ? dto?.ProposerDetails?.FirstName?.trim()
                            : ""
                        } ${
                          dto?.ProposerDetails?.MiddleName
                            ? `${dto?.ProposerDetails?.MiddleName?.trim()} `
                          ...
                          dto?.ProposerDetails?.LastName
                            ? dto.ProposerDetails?.LastName?.trim()
                            : ""
                        }`,
                        DOB: DateFormatFromDateObject(new Date(dto?.ProposerDetails?.DOB), "y-m-d"),
                        Age: dto?.ProposerDetails?.Age,
          ...
          const productDetails = {
            ...
            InsurableItem: [
              {
                ...dto.productDetails[productIndex].InsurableItem[0],
            ...
            InsurableItem: [
              {
                ...
                RiskItems: [
                  ...
                  {
                    ...(productDetails.InsurableItem[0]?.RiskItems[0] || {}),
                    Name: `${
                      dto?.ProposerDetails?.FirstName ? dto?.ProposerDetails?.FirstName?.trim() : ""
                    } ${
                      dto?.ProposerDetails?.MiddleName
    ...
    case 4:
      {
        ...
        if (res.finalResult && res.finalResult.PremiumDetails !== undefined) {
          ...
          //   {
          //     ...newProductDetails?.PremiumDetails,
          //     "Premium Mode": `${12 / (newProductDetails?.frequency || 1)}`,
          //     Nach: newProductDetails?.Nach !== "No",
          //   },
          //   ...(res.finalResult.AdditionalPremiumModes || []).sort(
          //     (a, b) => b["Premium Mode"] - a["Premium Mode"]
    ...
    case 5:
      dto.productDetails.forEach((e) => {
        if (
          checkForValue(e.PremiumDetails["Total Premium"]) ||
          e.PremiumDetails["Total Premium"] === "0"
        ) {
          fun = false;
          swal({
            text: "Total Premium is Zero",
            icon: "error",
          });
        } else {
          fun = true;
          ...
          navigate(`/lifeCustomerProposals?OpportunityId=${dto.ProposerDetails.opportunityId}`);
        }
      });
      break;
...
const getOnPreviousClick = async ({ activeStep, setActiveStep, dto, navigate }) => {
  let fun = true;
  let lActiveStep = activeStep;
  if (dto?.productDetails[0]?.InsurableItem[0]?.RiskItems[0]?.Benefit?.length <= 0) {
    if (activeStep === 5) {
  ...
  // if (dto?.ProposerDetails?.IsProposerSameAsInsured === "Yes") {
  //   if (lActiveStep === 3) lActiveStep = 1;
  // }
  switch (lActiveStep) {
    case 0:
      fun = true;
...
const getButtonDetails = ({ activeStep, dto, LoginStatus }) => {
  let btnDetails = {};
  let lActiveStep = activeStep;
  if (
    dto.productDetails &&
    dto?.productDetails[0]?.InsurableItem[0]?.RiskItems[0]?.Benefit?.length <= 0
  ) {
  ...
  switch (lActiveStep) {
    case 0:
      btnDetails = {
        prev: { label: "Back", visible: LoginStatus },
    ...
    case 5:
      btnDetails = {
        ...
        next: {
          ...
          visible: dto.productDetails.filter((e) => checkForValue(e.QuoteNo)).length === 0,
        },
...
const getMasterData = async ({
  ...
  if (!checkForValue(selectedId)) {
    setLoading(true);
    const opportunityData = await GetOpportunity(selectedId);
    setLoading(false);
    if (opportunityData.AdditionalDetailsJson?.AutoSave)
      setDto({
        ...opportunityData.AdditionalDetailsJson.AutoSave,
        productDetails: opportunityData.AdditionalDetailsJson.AutoSave.productDetails
          ? [...opportunityData.AdditionalDetailsJson.AutoSave.productDetails]
          : [{ InsurableItem: { RiskItems: [{ DOB: "" }] } }],
      });
    // setLoading(true);
