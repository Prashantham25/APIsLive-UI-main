...
function LayoutStructure({ prod, dto, setDto, masters, setMasters, genericInfo, genericMethods }) {
  const [menuIndex, setMenuIndex] = useState(0);
  const [nextFlag, setNextFlag] = useState(false);
  const [nextCount, setNextCount] = useState(0);
  const [midNextValidationId, setMidNextValidationId] = useState(-1);
  // [getTopLevelContent, getMenus, getAccordions, getControls, getPolicyDto, getMasters];

  const MenusList = Array.isArray(genericMethods?.[1]({ prod, dto, setDto, masters, setMasters }))
    ? genericMethods?.[1]({ prod, dto, setDto, masters, setMasters, genericInfo })
    : [];
  const TopContent = Array.isArray(genericMethods?.[0]({ prod, dto, setDto, masters, setMasters }))
    ? genericMethods?.[0]({
        prod,
        dto,
        setDto,
  ...
  const accordionList = Array.isArray(
    genericMethods?.[2]({ prod, menuIndex, dto, setDto, masters, setMasters })
  )
    ? genericMethods?.[2]({
        prod,
        menuIndex,
  ...
  const dyndata = Array.isArray(
    genericMethods?.[3]({ prod, menuIndex, dto, setDto, masters, setMasters })
  )
    ? genericMethods?.[3]({
        prod,
        menuIndex,
...
export default function DuplicateProcessingV2({ prodCode, genericMethods }) {
  const [control] = useDataController();
  const { genericInfo } = control;
  const [prod, setProd] = useState("Motor_PrivateCar_PClaimHistory");
  ...
  useEffect(() => {
    ...
    if (!checkForValue(prodCode)) setProd(prodCode);
  }, []);
  ...
  useEffect(async () => {
    if (prod !== "" && prod !== null && prod !== undefined) {
      const json = await genericMethods?.[4]({ prod, genericInfo });
      setDto(json);
      const mas = await genericMethods?.[5]({ dto: json, prod, genericInfo });
      console.log("mas", mas);
      setMasters({ ...mas });
