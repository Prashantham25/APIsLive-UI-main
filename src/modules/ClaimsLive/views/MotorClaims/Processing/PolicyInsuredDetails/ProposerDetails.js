...
function RenderControl({ item, Proposerdata, SetProposerdata }) {
  const ProposerdataL = Proposerdata;
  ...
  const handlechangeData = (e) => {
    ProposerdataL.PolicyInsuredDetails.ProposerDetails[e.target.name] = e.target.value;
    SetProposerdata((prev) => ({ ...prev, ...ProposerdataL }));
  };
  return (
    <div>
      {(() => {
          ...
          case "Input":
            return (
              <MDInput
                label={item.label}
                name={item.name}
                value={Proposerdata.PolicyInsuredDetails.ProposerDetails[item.name]}
...
function ProposerDetails() {
  const [Proposerdata, SetProposerdata] = useState(processingData);
  const controlItems = [
    {
      type: "Input",
      label: "Salution",
