...
function EstimationBilling({ setContent }) {
  ...
  const [benefit, setBenefit] = useState({ BenefitCriteria: "" });
  // const [ApprovedAmount, setApprovedAmount] = useState(0);
  const [ApiName, setApiName] = useState("");
  const [plan, setPlan] = useState("");
  const [si, setSI] = useState("");
  const [onSaveFlag, setOnSaveFlag] = useState(false);
  const [bsiData, setBsiDetails] = useState({});
  ...
  const Calculate = async () => {
    ...
    if (billType === "Lumpsum") {
      if (claimedAmount === "") {
        ...
        setbenefitDetails({
          ...benefitDetails,
          calculatedClaimAmount: result.finalResult.CalClaimAmount,
          deductible: result.finalResult.DeductibleValue,
        });
  ...
  useEffect(async () => {
    ...
    const ActivityType = "CalculateClaimAmount";
    ...
    console.log(r3);
    setSI(r3.policy_details[0].policyRequest.SumInsured);
    setPlan(r3.policy_details[0].policyRequest.Plan);
    const bsi = await GetBsiDetails(
      PolicyNumber,
      ClaimObj.basicDetails.memberDetails.memberId,
      ClaimObj.claimTransactionDTO[0].transactionDetailsDto.typeOfLoss
    );
    if (bsi.status === 1) {
      setBsiDetails(bsi.finalResult);
    }
    setOpenFlag(false);
  }, []);

  useEffect(() => {
    console.log("bsi", bsiData);
  }, [bsiData]);
  ...
  useEffect(() => {
    if (TravelEnquiryFlag || onSaveFlag) {
      ...
      if (
        ClaimObj.claimTransactionDTO[0].transactionDetailsDto.benefitDetails[0].isLumpSum === "Yes"
      ) {
        setBillType("Lumpsum");
        setClaimedAmount(
        ...
        const arr2 = [
          ...ClaimObj.claimTransactionDTO[0].transactionDetailsDto.benefitDetails[0].billingDetails,
        ];
  ...
  const onSave = async () => {
    ...
    benefitDetails.calculatedClaimAmount = CalculatedData.CalClaimAmount;
    benefitDetails.claimedAmount = claimedAmount;
    benefitDetails.deductible = CalculatedData.Deductible;
  ...
  return (
    <MDBox>
      ...
      >
        ...
        <AccordionDetails>
          {benefit.BenefitType !== "Benefit" || benefit.BenefitType !== "benefit" ? (
            <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
              <h3>
                Balance Sum Insured(BSI) :{bsiData.BSI}
                &nbsp;
                {benefit.BenefitCurrencyType}
              </h3>
            </Grid>
          ) : null}
      ...
      <Backdrop sx={{ color: "#fff", zIndex: (theme) => theme.zIndex.drawer + 1 }} open={openflag}>
        <img
          alt=""
          src={loader}
