...
import LoginImage from "assets/images/Takaful/LoginImage.jpg";
import Logo from "assets/images/Takaful/Logo.png";
...
function TakafulOmanLoginPage() {
      ...
      if (userData.status === 200) {
        if (userData.data.status === 1) {
          ...
          if (login.data.status === 1) {
            ...
            else Navigate(`/Claims/SearchClaims`);
          } else {
            Swal.fire({
              text: "Wrong Password",
              icon: "error",
  ...
  const onForgotPassword = async () => {
    if (Logindata.UserName === "") {
      Swal.fire({ icon: "error", text: "Please enter User ID", allowOutsideClick: false });
    } else {
      const userData = await getUserType(Logindata);
  ...
  return (
    <Card>
      <Grid container>
        <Grid item xs={12} sm={12} md={8} lg={8} xl={8} sx={{ position: "relative" }}>
          <MDBox
            width="100%"
            height="100vh"
          ...
          >
            <img src={LoginImage} alt="bg" width="100%" />
            <MDBox
              sx={{
                width: "100%",
                bgcolor: "#b9c796",
        ...
        <Grid item xs={12} sm={12} md={4} lg={4} xl={4}>
          <MDBox width="100%" height="100vh" sx={{ display: "flex", alignItems: "center" }}>
            <Grid container spacing={2} p={3}>
              <MDBox width="189px%" height="71px%" ml={18}>
...
export default TakafulOmanLoginPage;
